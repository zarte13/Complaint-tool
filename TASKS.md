...
## Future Features (i18n) 📋
### Task Group: Internationalized Responsables & Issue Subtypes
**Status**: 📋 **PLANNED**  
...
| Task ID | Description | Prerequisites | Acceptance Criteria | Effort | Status | Related Files |
|---------|-------------|---------------|-------------------|--------|--------|---------------|
| **FF-001** | Responsables directory page (i18n) | DA-004 | • Route `/responsables` lists all responsables with search/sort/pagination<br>• Add/Edit person with fields: `name` (required, 1–255), `email` (required, valid email)<br>• Activate/Deactivate person; deactivated hidden by default with toggle to show<br>• EN/FR translations for labels, buttons, validation messages<br>• Backend: top-level endpoints `/api/responsible-persons/` (GET list with `search`, POST create, PUT update, DELETE deactivate) while preserving existing nested usage under actions<br>• DB: `responsible_persons` table (id, name, email unique, active bool default true, created_at, updated_at) or migration to align if already present<br>• TypeScript: `Person` interface and API client methods<br>• Accessibility: keyboard nav, ARIA for modals/forms | 1–1.5 days | 📋 | frontend: `pages/ResponsablesPage.tsx`, `components/Responsables/` (List, Form), `i18n/translations.ts`; backend: `app/api/responsibles.py`, `app/models/models.py`, `app/schemas/schemas.py`, `migrate_db.py` |
| **FF-002** | Issue types with subtypes and dynamic fields (i18n) | FE-008, DA-003 | • Replace `issue_type` choices with categories: `Dimensional`, `Visual`, `Packaging`, `Other` (keys normalized as `dimensional`, `visual`, `packaging`, `other`)<br>• Subtypes: for `visual` → `scratch`, `nicks`, `rust`; for `packaging` → `wrong_box`, `wrong_bag`, `wrong_paper`, `wrong_part`, `wrong_quantity`, `wrong_tags` (multi-select allowed)<br>• UI: multiple subtypes selectable; conditional inputs appear per selected packaging subtype:<br>  - For `wrong_box`, `wrong_bag`, `wrong_paper`, `wrong_quantity`: show two inputs each — "Received" and "Expected" (mirrors current wrong_quantity UX)<br>• Validation: values required only for selected packaging subtypes; retain existing quantity rules<br>• Persistence: add `issue_category` (enum) and `issue_subtypes` (JSON array) to complaints; store packaging details as JSON objects `packaging_received` and `packaging_expected` keyed by subtype<br>• Filtering/export unaffected; API schemas updated; migrations provided<br>• EN/FR translations for categories, subtypes, helper texts, validation messages | 2–3 days | 📋 | frontend: `components/ComplaintForm/ComplaintForm.tsx`, `components/ComplaintDetailDrawer/EnhancedComplaintDetailDrawer.tsx`, `types/index.ts`, `i18n/translations.ts`; backend: `app/schemas/schemas.py`, `app/models/models.py`, `app/api/complaints.py`, `migrate_db.py` |
| **FF-003** | Responsables ↔ follow-up actions integration (i18n) | DA-004, FF-001 | • On `/responsables`, each person row shows count by action status and an expandable panel listing linked follow-up actions with: action title, status, due date, and linked complaint ID (click navigates to complaint drawer)<br>• Filters: by action status, overdue only<br>• Backend: endpoint `/api/responsible-persons/{id}/actions` with filters; indexes ensure performant queries<br>• EN/FR translations for headers, filters, empty states<br>• Tests: unit (frontend), API (backend), and integration for expansion behavior | 1–2 days | 📋 | frontend: `pages/ResponsablesPage.tsx`, `components/Responsables/ActionsPanel.tsx`, `services/api.ts`; backend: `app/api/actions.py` or `app/api/responsibles.py` (joined view), `app/models/models.py` |
